...
def get_tiles(img,img0=None):
    coords = compute_coords(img,
                            patch_size=sz,
                            precompute=True,
                            min_patch_info=0.35,
                            min_axis_info=0.35,
    ...
    if img0 is None:
        for (v, y, x) in coords:
            p = img[y:y+sz,x:x+sz]
            if p.shape[0] != sz or p.shape[1] != sz: continue
            tiles.append(p)
    else:
        for (v, y, x) in coords:
            p = img0[(4*y):(4*y+4*sz),(4*x):(4*x+4*sz)]
            if p.shape[0] != 4*sz or p.shape[1] != 4*sz: continue
